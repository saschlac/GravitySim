var Mass = function (initX, initY, veloX, veloY, mass, density, color, layer, type, parentSim) {
...
Mass.prototype.updatePosition = function(masses){
    ...
    if(this.parentSim.borders){
        if( newX + this.getRadius() > this.stage.getWidth() ){
        ...
        if( newY + this.getRadius() > this.stage.getHeight() ){
...
Mass.prototype.setPosition = function(x,y){
    this.position.x = x;
    this.position.y = y;
    this.circle.setPosition(x, this.position.y);
}
